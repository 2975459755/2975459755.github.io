...
- 删除***非底层节点***的元素：按照排序树的性质，和底层的前驱/后继交换，然后转化为底层节点元素的删除
...
1. 开始时，一次性读满内存工作区，$minimax = -\inf$
2. 选择内存的最小元素
    - 如果大于minimax，输出到输出缓冲区，更新minimax的值
    - 如果小于minimax，将该数据标红，不再将他选中，然后重新开始步骤2
3. 从外存读入第一个元素到内存，返回步骤2
4. 当内存工作区全部标红时（都小于minimax），生成一个归并段，然后将红色取消，开始下一个归并段的构建

|![置换选择排序0](置换选择排序0.png)|![置换选择排序1](置换选择排序1.png)|
|![置换选择排序2](置换选择排序2.png)|![置换选择排序3](置换选择排序3.png)|
|![置换选择排序4](置换选择排序4.png)|![置换选择排序5](置换选择排序5.png)|
